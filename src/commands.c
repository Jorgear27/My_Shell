...
char* internal_commands[] = {"cd", "clr", "echo", "quit", "help", "search", NULL};
...
void show_help(void)
{
    ...
    printf("  search <dir> <ext> - Searches for files with a specific extension "
           "in a directory\n");
    ...
    printf("  show_config       - Shows the current configuration of the "
           "monitoring program\n");
    printf("  update_config <key> <value> - Updates the configuration of the "
           "monitoring program\n");
}
    ...
    if (is_internal_command(args))
    {
        ...
        else if (strcmp(args[0], "search") == 0)
        {
            if (args[1] != NULL && args[2] != NULL)
            {
                ...
                char* ext = strtok(args[2], " ");
                if (dir && ext)
                {
                    search_files(dir, ext, 0);
                }
            }
            else
            {
                printf("Usage: search <directory> <extension>\n");
            }
        }
    }
...
void execute_piped_commands(char* input)
{
    ...
    for (int i = 0; i < num_commands; i++)
    {
        ...
        if (pid == 0)
        {
            ...
            {
                dup2(prev_fd,
                     STDIN_FILENO); // Redirect the input from the previous command
            ...
            {
                dup2(pipe_fds[1],
                     STDOUT_FILENO); // Redirect the output to the next command
            }
            ...
            parse_input(commands[i], args, &input_file,
                        &output_file); // Divide the command into arguments
...
void search_files(const char* dir_path, const char* file_extension, int printed)
{
    struct dirent* entry;
    struct stat statbuf;
    char path[PATH_MAX];
    ...
    while ((entry = readdir(dp)) != NULL)
    {
        ...
        if (stat(path, &statbuf) == 0 && S_ISDIR(statbuf.st_mode))
        {
            ...
            {
                search_files(path, file_extension, printed);
            }
        }
        else if (stat(path, &statbuf) == 0 && S_ISREG(statbuf.st_mode))
        {
